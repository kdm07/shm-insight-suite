...
const instrHod: Employee = {
  id: "EMP003",
  name: "Dr. Vinod Prakash",
  designation: "Head — Instrumentation",
  role: "Instrumentation HOD",
  department: "Instrumentation",
  email: "vinod.p@kdmshm.com",
  phone: "+91 98450 32345",
...
const numHod: Employee = {
  ...
  designation: "Head — Numerical Analysis",
  role: "Numerical HOD",
  department: "Numerical",
  email: "lakshmi.n@kdmshm.com",
  phone: "+91 98450 42345",
...
export const projects: Project[] = bridges.map((b, i) => {
  ...
  const start = new Date(2025, i % 12, 1 + (i % 20));
  const end = new Date(2026, (i + 4) % 12, 15 + (i % 10));
  const workflowStep =
    status === "Planning" ? 2 :
    status === "Running" ? 5 :
    status === "Review" ? 6 :
  ...
  return {
    ...
    progress,
    health: status === "Delayed" ? "Critical" : status === "Review" || progress > 70 ? "Good" : "At Risk",
...
projects.forEach((p) => {
  [projectManager, instrHod, numHod].forEach((e) => e.projects.push(p.id));
...
export const tasks: Task[] = Array.from({ length: 80 }).map((_, i) => {
  ...
  const due = new Date(Date.now() + ((i % 20) - 5) * 86400000);
  return {
...
// ---------- Documents ----------
const docTypes: DocumentItem["type"][] = ["Report", "Drawing", "Site Photo", "Sensor Layout", "PDF"];
export const documents: DocumentItem[] = Array.from({ length: 24 }).map((_, i) => ({
  id: `DOC${String(i + 1).padStart(3, "0")}`,
  name: `${pick(projects, i).bridgeName} — ${pick(docTypes, i)} ${i + 1}.pdf`,
  projectId: pick(projects, i).id,
  type: pick(docTypes, i),
  size: `${(0.4 + (i % 8) * 0.35).toFixed(1)} MB`,
  uploadedBy: pick(employees, i + 3).name,
  uploadedAt: new Date(Date.now() - i * 86400000).toISOString().slice(0, 10),
}));
